...
describe('NgModule-example', function () {
  ...
  function getContactSectionStruct() {
    const buttons = element.all(by.css('app-address-book form button'));

    return {
      header: element.all(by.css('app-address-book h2')).get(0),
      popupMessage: element.all(by.css('app-address-book div')).get(0),
      contactNameHeader: element.all(by.css('app-address-book form h3')).get(0),
      input: element.all(by.css('app-address-book form input')).get(0),
      validationError: element.all(by.css('app-address-book form .alert')).get(0),
  ...
  function contactTests(color: string, name?: string) {
    return function() {
      it('shows the address-book\'s owner', function() {
      ...
      it('can create a new address-book', function () {
        const contacts = getContactSectionStruct();
  ...
  describe('index.html', function () {
    ...
    describe('address-book', contactTests(lightgray, 'Miss Marple'));
    ...
    describe('address-book-list', function () {
      beforeEach(function () {
        getCommonsSectionStruct().customersButton.click();
      });

      it('shows a list of address-book-list', function() {
